...
app.get("/api/materials", async (req, res) => {
	if (!process.env.SAP_HOST) {
		return res.status(503).json({
			success: false,
			message: "He thong SAP chua duoc cau hinh (thieu SAP_HOST)."
		});
	}
	try {
		const response = await axios.get(
			`${process.env.SAP_HOST}${ODATA_SERVICE_PATH}/MaterialSet`,
			{ params: { "$format": "json" }, auth: sapAuth(), timeout: 25000 }
		);
		const results = (response.data && response.data.d && response.data.d.results) || [];
		const data = results.map(function (row) {
			return {
				MaterialNo: String(row.MaterialNo || row.Matnr || row.MATERIAL || row.Product || "").trim(),
				MaterialType: String(row.MaterialType || row.Mtart || row.MTART || "").trim(),
				Description: String(row.Description || row.Maktx || row.MAKTX || row.ProductDescription || "").trim(),
				BaseUoM: String(row.BaseUoM || row.BaseUnit || row.Meins || row.MEINS || "PC").trim()
			};
		}).filter(function (m) { return !!m.MaterialNo; });

		console.log("[SAP] MaterialSet:", results.length, "→ normalized:", data.length);
		return res.json({ success: true, data: data });
	} catch (error) {
		const detail = error.response && error.response.data
			? JSON.stringify(error.response.data).slice(0, 400)
			: error.message;
		console.error("❌ MaterialSet:", detail);
		return res.status(502).json({
			success: false,
			message: "Khong the lay du lieu vat tu tu SAP.",
			sapError: true
		});
	}
});
